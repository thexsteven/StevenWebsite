...
export default function Hawaii() {
  return <>
    ...
    <section className="story-grid wrap" data-sc-act="flow"><MediaPlaceholder name="hawaii" caption="Vorgesehen: der Auftakt auf Oahu." /><nav className="chapter-nav" aria-label="Hawaii-Kapitel"><h2>Die Kapitel</h2>{chapters.map((chapter) => <Link key={chapter.slug} href={`/reisen/hawaii/${chapter.slug}`}>{chapter.title}<span aria-hidden="true">↗</span></Link>)}</nav></section>
