...
const renderMediaLine = (line) => {
  ...
  if (kind === 'video') {
    ...
    return `<figure class="story-figure"><video ${attrs} poster="${poster}"><source src="${src}" type="video/mp4">Dein Browser unterstuetzt kein Video-Tag.</video>${captionHtml}</figure>`;
  }
  const src = cloudinaryImageUrl(publicId, opts);
  const alt = caption ? escapeHtml(caption) : '';
  return `<figure class="story-figure"><img src="${src}" alt="${alt}" loading="lazy" />${captionHtml}</figure>`;
};
